...
** Go back to beeline terminal and now rerun the `CREATE TABLE` command. This time this should succeed.
...
----
+
TIP: For every new login as `etl_user` , ensure that you `kinit` the session.
